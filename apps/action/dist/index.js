...
// src/trustsignal/types.ts
var trustSignalVerificationRequestSchema = external_exports.object({
  apiVersion: external_exports.literal("2026-03-13"),
  ...
  subject: external_exports.object({
    kind: external_exports.enum(["workflow_run", "release", "commit"]),
    summary: external_exports.string().min(1)
  }).strict(),
  repository: external_exports.object({
    owner: external_exports.string().min(1),
    repo: external_exports.string().min(1),
    fullName: external_exports.string().min(3),
    defaultBranch: external_exports.string().min(1).optional(),
    htmlUrl: external_exports.string().url().optional()
  }).strict(),
  provenance: external_exports.object({
    eventName: external_exports.enum(["workflow_run", "release", "push"]),
    attributes: external_exports.record(external_exports.string(), external_exports.string())
  }).strict()
}).strict();
var trustSignalVerificationResponseSchema = external_exports.object({
  status: external_exports.enum(["queued", "in_progress", "completed"]),
  conclusion: external_exports.enum(["success", "failure", "neutral"]).optional(),
  title: external_exports.string().min(1),
  summary: external_exports.string().min(1),
  detailsUrl: external_exports.string().url().optional(),
  receiptId: external_exports.string().min(1).optional(),
  verificationTimestamp: external_exports.string().datetime({ offset: true }),
  provenanceNote: external_exports.string().min(1)
...
function normalizeProvenanceAttributes(input) {
  return Object.fromEntries(
    Object.entries(input).filter(([, value]) => value !== void 0).sort(([left], [right]) => left.localeCompare(right)).map(([key, value]) => [key, String(value)])
  );
}
function buildTrustSignalVerificationRequest(envelope) {
  const request = {
...
function normalizeWorkflowRunPayload(payload) {
...
var actionEnvSchema = external_exports.object({
  GITHUB_EVENT_NAME: external_exports.enum(["workflow_run", "release", "push"]),
  GITHUB_EVENT_PATH: external_exports.string().min(1),
  TRUSTSIGNAL_API_BASE_URL: external_exports.string().url(),
  TRUSTSIGNAL_API_KEY: external_exports.string().min(1),
  GITHUB_OUTPUT: external_exports.string().optional()
});
function parseActionEnv(input) {
...
function readGitHubEventPayload(eventPath) {
...
function setOutput(name, value) {
  ...
  (0, import_node_fs2.appendFileSync)(process.env.GITHUB_OUTPUT, `${name}=${value}
`);
}
async function runAction(envInput = process.env) {
  ...
  const envelope = normalizeGitHubEventToEnvelope({
    ...
    action: typeof payload.action === "string" ? payload.action : void 0,
    payload
  });
  if (!envelope) {
